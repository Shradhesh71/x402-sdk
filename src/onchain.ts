import { ExecResponse, SDKConfig, X402PaymentRequirements, PaymentConfig, TokenType } from './types';
 ...
 */
export async function submitOnChainDirect(
  paymentReq: X402PaymentRequirements, 
  opts: { payerKeypair: Keypair; network: SDKConfig['network']; toPubkey?: string }
): Promise<ExecResponse> {
  const network = opts.network === 'solana-mainnet' ? 'mainnet-beta' : 'devnet';
  const conn = new Connection(clusterApiUrl(network as any), 'confirmed');
  const payer = opts.payerKeypair;
  const to = opts.toPubkey ? new PublicKey(opts.toPubkey) : payer.publicKey;
 ...
 */
export async function submitPaymentOnChain(
  ...
  const connection = getConnection(opts.network);
  const payer = opts.payerKeypair;
  ...
  try {
    if (config.tokenType === 'SOL') {
      transaction = await createSOLPaymentTransaction(connection, payer, config);
    } else {
      if (!config.mintAddress) {
        if (config.tokenType === 'USDC') {
          config.mintAddress = getUSDCMint(opts.network);
        } else {
          throw new Error(`Mint address is required for ${config.tokenType} payments`);
        }
      }
      transaction = await createSPLPaymentTransaction(connection, payer, config);
    }
...
export async function createSignedPaymentTransaction(
  config: PaymentConfig,
  opts: { payerKeypair: Keypair; network: SDKConfig['network'] }
): Promise<{
